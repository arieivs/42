...
bool	ClapTrap::hasVitals(void) {
	if (this->_energy_points <= 0) {
		std::cout << this->_name << " ran out of energy points..." << std::endl;
		return (false);
	}
	if (this->_hit_points <= 0) {
		std::cout << this->_name << " ran out of hit points..." << std::endl;
		return (false);
	}
	return (true);
}
...
void	ClapTrap::beRepaired(unsigned int amount) {
	...
	std::cout << this->_name << " won " << amount << " hit points, it has "
			  << this->_hit_points << " hit points now." << std::endl;
}
